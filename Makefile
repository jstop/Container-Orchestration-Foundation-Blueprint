...
bootstrap:
	...
	cd $(CDK_PATH) && . ${NVM_DIR}/nvm.sh && nvm install && nvm use && npm install && npx cdk bootstrap aws://$(AWS_ACCOUNT)/$(AWS_REGION) -c domainName=$(HOSTED_ZONE_NAME) -c platformTeamUserRoleArn=$(PLATFORM_TEAM_USER_ROLE_ARN)
